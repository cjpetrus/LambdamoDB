...
class Model(object):
    ...
    def create_table(cls):
        ...
        table_attributes = ['TableName', 'KeySchema', 'AttributeDefinitions', 'ProvisionedThroughput',
        ...
        # TODO, i need to use an ordered dict here.
    ...
    def query(cls, **params):
        ...
        fields = ['TableName', 'ConsistentRead', 'ExclusiveStartKey', 'ExpressionAttributeNames',
...
class Key:
    ...
    def key_type(self):
        if self._key_type in ['hash', 'range']:
            ...
            raise WrongKeyTypeError(self._key_type,
                                    '{} is not a valid key type. Valid types are hash and range.'.format(
                                        self._key_type))
    ...
    def attr_type(self):
        if self._attr_type in ['N', 'S', 'B']:  # Make sure there are no more types.
            ...
            raise WrongAttributeTypeError(self._attr_type,
                                          '{} is not a valid attribute type. Valid types are N and S'.format(
                                              self._attr_type))
